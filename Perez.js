...
const packname = process.env.STICKER_PACKNAME || '⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻';
const dev = process.env.DEV || '254781346242';
...
async function handleMessageRevocation(client, revocationMessage, targetChat = null) {
...
contextInfo: {
      ...
      await client.sendMessage(sendTo, { audio: buffer, ptt: isPTT, mimetype: 'audio/mpeg', 
contextInfo: {
          externalAdReply: {
          title: notificationText,
...
// DEBUGGED: Fixed antidelete toggle logic with chat destination options
if (antidelete !== "off") {
    if (mek.message?.protocolMessage?.key) {
        // This is a message deletion event
        if (antidelete === "chat" && !m.isGroup) {
            // Private chats only - send to current chat
            await handleMessageRevocation(client, mek, m.chat);
        } else if (antidelete === "group" && m.isGroup) {
            // Group chats only - send to current group
            await handleMessageRevocation(client, mek, m.chat);
        } else if (antidelete === "private") {
            // Send to private chat only
            await handleMessageRevocation(client, mek, client.user.id);
        } else if (antidelete === "both") {
            // Send to both current chat and private chat
            await handleMessageRevocation(client, mek, m.chat);
            await handleMessageRevocation(client, mek, client.user.id);
        } else if (antidelete === "on") {
            // Backward compatibility - send to private chat
            await handleMessageRevocation(client, mek, client.user.id);
        }
    } else {
        // This is a regular incoming message - store it
        if (antidelete === "chat" && !m.isGroup) {
            // Store private messages only
            handleIncomingMessage(mek);
        } else if (antidelete === "group" && m.isGroup) {
            // Store group messages only
            handleIncomingMessage(mek);
        } else if (antidelete === "private" || antidelete === "both") {
            // Store for private or both modes
            handleIncomingMessage(mek);
        } else if (antidelete === "on") {
            // Store both chat types (backward compatibility)
            handleIncomingMessage(mek);
        }
    }
}
...
client.sendMessage(m.chat, { image: { url: 'https://files.catbox.moe/z6k2ho.jpg' }, caption: cap, fileLength: "9999999999"}, { quoted: m })
} else if (menu === 'LINK') {
client.sendMessage(m.chat, {
                        text: cap,
                        contextInfo: {
                            externalAdReply: {
                                ...
                                sourceUrl: 'https://wa.me/254781346242?text=Hello👋+perez+Nihostie+Bot+Mkuu+😔',
                                mediaType: 1,
                                renderLargerThumbnail: true
                            }
                        }
...
break;	      	      
    ...
    await client.sendMessage(m.chat, {
      ...
      caption: `GENERATED BY ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻`
    });
  } catch (_0180d034) {
    m.reply(_0180d034);
  }
...
break;
  ...
  break;
	case 'leaves': {
		     var mumaker = require("mumaker"); 
		      if (!text || text == "") {
      m.reply("Example Usage : " + prefix + "Leaves ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻");
      return;
    }
	try {
...
m.reply("*Wait a moment...*");
    ...
    if (!args || !args[0]) {
      ...
      setTimeout(() => {
        client.sendMessage(m.chat, {
          text: `⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻 will now remove all ${_0x2f8982.length} foreigners from this group chat in the next second.\n\nGood bye Foreigners. 😔`
        }, {
          quoted: m
        });
        setTimeout(() => {
  ...
  try {
    ...
    if (!text) {
      return m.reply("𝗣𝗿𝗼𝘃𝗶𝗱𝗲 𝘀𝗼𝗺𝗲 𝗶𝗻𝘀𝘁𝗿𝘂𝗰𝘁𝗶𝗼𝗻𝘀 𝗲𝗵! 𝗧𝗵𝗶𝘀 𝗶𝘀 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻 AI, 𝘂𝘀𝗶𝗻𝗴 𝗴𝗲𝗺𝗶𝗻𝗶-𝗽𝗿𝗼-𝘃𝗶𝘀𝗶𝗼𝗻 𝘁𝗼 𝗮𝗻𝗮𝗹𝘆𝘀𝗲 𝗶𝗺𝗮𝗴𝗲𝘀.");
    }
    if (!/image/.test(mime)) {
...
let fdr = await client.downloadAndSaveMediaMessage(m.quoted)
                    ...
                    m.reply("𝗔 𝗠𝗼𝗺𝗲𝗻𝘁, [⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻-𝗔𝗜] 𝗶𝘀 𝗮𝗻𝗮𝗹𝘆𝘇𝗶𝗻𝗴 𝘁𝗵𝗲 𝗰𝗼𝗻𝘁𝗲𝗻𝘁𝘀 𝗼𝗳 𝘁𝗵𝗲 𝗶𝗺𝗮𝗴𝗲. . .");
...
break;		      
	      case "ai3": {
		      if (!msgDreaded || !text) {
    m.reply("𝗤𝘂𝗼𝘁𝗲 𝗮𝗻 𝗶𝗺𝗮𝗴𝗲 𝗮𝗻𝗱 𝗴𝗶𝘃𝗲 𝘀𝗼𝗺𝗲 𝗶𝗻𝘀𝘁𝗿𝘂𝗰𝘁𝗶𝗼𝗻𝘀 𝗲𝗵. 𝗜'𝗺 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻 𝗔𝗶, 𝗶 𝘂𝘀𝗲 𝗕𝗮𝗿𝗱 𝘁𝗼 𝗮𝗻𝗮𝗹𝘆𝘇𝗲 𝗶𝗺𝗮𝗴𝗲𝘀.");
    return;
  }
  ;
  let _0x44b3e0;
  ...
  try {
    ...
    m.reply("An error occured\n" + _0x1be711);
	 ...
	 break;
			...
			client.sendMessage(m.chat, { image: proses, caption: '𝗚𝗲𝗻𝗲𝗿𝗮𝘁𝗲𝗱 𝗯𝘆 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻'}, { quoted: m })
			}
			break;
...
await client.sendMessage(m.chat, { image: { url: imageurl}, caption: `𝗖𝗼𝗻𝘃𝗲𝗿𝘁𝗲𝗱 𝗯𝘆 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻 `}, { quoted: m}) 
...
case "faker": {
    ...
    if (!args || !args[0]) {
      ...
      let _0x2d7d67 = `Nexus 𝚑𝚊𝚜 𝚍𝚎𝚝𝚎𝚌𝚝𝚎𝚍 𝚝𝚑𝚎 𝚏𝚘𝚕𝚕𝚘𝚠𝚒𝚗𝚐 ${_0x2f8982.length} 𝙵𝚊𝚔𝚎 𝚊𝚌𝚌𝚘𝚞𝚗𝚝𝚜 𝚒𝚗 𝚝𝚑𝚒𝚜 𝚐𝚛𝚘𝚞𝚙:- \n`;
      for (let _0x28761c of _0x2f8982) {
...
case 'save': {
  ...
  if (Owner && quotedMessage && textL.startsWith(prefix + "save") && m.quoted.chat.includes("status@broadcast")) {
    try {
      ...
      await m.reply("❌ Failed ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻.");
    }
  }
}
break;			      
...
const cap = "𝗘𝗱𝗶𝘁𝗲𝗱 𝗯𝘆 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻";
...
let fdr = await client.downloadAndSaveMediaMessage(m.quoted)
                    ...
                    m.reply("𝗔 𝗺𝗼𝗺𝗲𝗻𝘁, ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻 is 𝗲𝗿𝗮𝘀𝗶𝗻𝗴 𝘁𝗵𝗲 𝗯𝗮𝗰𝗸𝗴𝗿𝗼𝘂𝗻𝗱. . .");
...
break;
              ...
              client.sendMessage(m.chat, { image: { url: 'https://files.catbox.moe/k2u5ks.jpg' }, caption:`*𝐁𝐎𝐓 𝐍𝐀𝐌𝐄: ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻*\n\n*𝐒𝐏𝐄𝐄𝐃: ${dreadedspeed.toFixed(4)} 𝐌𝐒*\n\n*𝐑𝐔𝐍𝐓𝐈𝐌𝐄: ${runtime(process.uptime())}*\n\n*𝐏𝐋𝐀𝐓𝐅𝐎𝐑𝐌: 𝗛𝗲𝗿𝗼𝗸𝘂*\n\n*𝐇𝐎𝐒𝐓𝐍𝐀𝐌𝐄: 𝗥𝗮𝘃𝗲𝗻*\n\n*𝐋𝐈𝐁𝐑𝐀𝐑𝐘: Baileys*\n\n𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑: 𝗡𝗶𝗰𝗸༆`}); 
 break;
...
await client.sendMessage(m.chat, { image: { url: imagelink}, caption: "Converted by ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻! 🦄"}, { quoted: m});
...
m.reply("𝗣𝗲𝗻𝗱𝗶𝗻𝗴 𝗣𝗮𝗿𝘁𝗶𝗰𝗶𝗽𝗮𝗻𝘁𝘀 𝗵𝗮𝘃𝗲 𝗯𝗲𝗲𝗻 𝗿𝗲𝗷𝗲𝗰𝘁𝗲𝗱!");
 ...
 if (!Owner) throw NotOwner;  
     ...
     let stoy = '*𝗕𝗲𝗹𝗼𝘄 𝗔𝗿𝗲 𝗛𝗲𝗿𝗼𝗸𝘂 𝗩𝗮𝗿𝗶𝗮𝗯𝗹𝗲𝘀 𝗙𝗼𝗿 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻:*\n\n';  
...
await client.sendMessage(m.chat,{video : {url : twtvid },caption : `𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗𝗘𝗗 𝗕𝗬 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻`,gifPlayback : false },{quoted : m}) 
...
if (!text) {
    ...
    try {
        ...
        await client.sendMessage(
            ...
            {
                video: { url: fbvid },
                caption: "𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗𝗘𝗗 𝗕𝗬 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻",
                gifPlayback: false,
            },
            { quoted: m }
        );
...
case "update": case "redeploy": {
        ...
        async function redeployApp() {
            try {
                ...
                await m.reply("Your bot is undergoing a ruthless upgrade, hold tight for the next 2 minutes as the redeploy executes! Once done, you’ll have the freshest version of *AstaxNexus MD* unleashed upon you.");
...
│
│ 💠 Unleash the power of ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻 — fast, sleek, unstoppable ⚡
...
│
│ 🌍 Enjoy the brilliance of ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻
...
try {
        ...
        const capp = `𝑫𝑶𝑾𝑵𝑳𝑶𝑨𝑫𝑬𝑫 𝑩𝒀 ⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻`;
...
if (!m.quoted) return m.reply("quote a viewonce message eh")
  ...
  const quotedMessage = m.msg?.contextInfo?.quotedMessage;
    ...
    if (quotedMessage.imageMessage) {
      ...
      client.sendMessage(m.chat, { image: { url: imageUrl }, caption: `Retrieved by AstaxNexus MD!\n${imageCaption}`}, { quoted: m });
    }

    if (quotedMessage.videoMessage) {
      let videoCaption = quotedMessage.videoMessage.caption;
      let videoUrl = await client.downloadAndSaveMediaMessage(quotedMessage.videoMessage);
      client.sendMessage(m.chat, { video: { url: videoUrl }, caption: `Retrieved by AstaxNexus MD!\n${videoCaption}`}, { quoted: m });
    }
...
break;  
 ...
 break;
		      case 'runtime':
		let raven = `⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻𝘿 𝗵𝗮𝘀 𝗯𝗲𝗲𝗻 𝗿𝘂𝗻𝗻𝗶𝗻𝗴 𝘀𝗶𝗻𝗰𝗲 ${runtime(process.uptime())}`
                client.sendMessage(m.chat, {
                    text: raven,
                    contextInfo: {
                        externalAdReply: {
...
case "movie": 
 ...
 break;
    ...
    break;
         ...
         for (let i of res) { 
             let txt = `⚡ 𝔸𝕤𝕥𝕒𝕩ℕ𝕖𝕩𝕦𝕤 𝕄𝔻 𝗕𝗥𝗢𝗔𝗗𝗖𝗔𝗦𝗧 >\n\n🀄 Message: ${text}\n\nAuthor: ${pushname}` 
...
try {
        ...
        pp = 'https://files.catbox.moe/0uzc42.jpg';
      }
